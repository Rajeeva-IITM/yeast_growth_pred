...
class NDCG(Metric):
    ...
    def dcg_score(self, y_true: torch.Tensor, y_score: torch.Tensor) -> torch.Tensor:
    ...
    def ndcg_score(self, y_true: torch.Tensor, y_score: torch.Tensor) -> torch.Tensor:
...
class PrecisionAtRecall(Metric):
    ...
    def update(self, distances: torch.Tensor, labels: torch.Tensor) -> None:
        ...
        threshold_index = torch.where(
            torch.cumsum(labels, dim=0) >= self.recall_point * torch.sum(labels)
        )
    ...
    def compute(self) -> torch.Tensor:
        return self.correct.float() / (self.correct + self.wrong + FLOAT32_EPSILON)
