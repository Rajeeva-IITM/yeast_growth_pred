...
class MultiViewNet(nn.Module):
    ...
    def __init__(
        ...
        # postconcatenation
        self.postconcat = [
            nn.Linear(layers_before_concat[-1]*2,
                      layers_after_concat[0],
                      dtype=torch.float),
        ...
        for i in range(len(self.layers_before_concat)-2):
            ...
            self.view1.append(
                nn.Linear(self.layers_before_concat[i], self.layers_before_concat[i+1], dtype=torch.float)
            )
            self.view2.append(
                nn.Linear(self.layers_before_concat[i], self.layers_before_concat[i+1], dtype=torch.float)
            )
        ...
        self.view1.append(
            nn.Linear(self.layers_before_concat[-2], self.layers_before_concat[-1], dtype=torch.float)
            )
        self.view2.append(
            nn.Linear(self.layers_before_concat[-2], self.layers_before_concat[-1], dtype=torch.float)
        )
        ...
        self.postconcat.append(
            nn.Linear(self.layers_after_concat[-2], self.layers_after_concat[-1], dtype=torch.float)
        )
