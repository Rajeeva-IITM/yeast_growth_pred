...
from code.FGR.src.data.components.utils import (
    smiles2vector_fg,
    smiles2vector_mfg,
    standardize_smiles,
)
from code.FGR.src.models.components.autoencoder import FGRPretrainModel
from code.FGR.src.models.fgr_module import FGRPretrainLitModule

fgroups = pd.read_parquet("/home/rajeeva/Project/yeast_growth_pred/code/FGR/fg.parquet")[
    "SMARTS"
].tolist()  # Get functional groups
fgroups_list = [MolFromSmarts(x) for x in fgroups]  # Convert to RDKit Mol
tokenizer = Tokenizer.from_file(
    os.path.join(
        "/home/rajeeva/Project/yeast_growth_pred/code/FGR/tokenizers",
...
def get_fgr_model(ckpt_path: str):
    ...
    model = FGRPretrainModel.load_state_dict(torch.load(ckpt_path))
    model.eval()
    return model
